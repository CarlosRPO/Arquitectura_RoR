...
class TokensControllerTest < ActionController::TestCase
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post :create, token: { number_of_request: @token.number_of_request, status: @token.status, token: @token.token }
    end
  ...
  test "should update token" do
    patch :update, id: @token, token: { number_of_request: @token.number_of_request, status: @token.status, token: @token.token }
    assert_redirected_to token_path(assigns(:token))
  end
